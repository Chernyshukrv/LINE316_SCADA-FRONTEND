...
## Формирование Docker-контейнера, загрузка проекта MasterSCADA
Для формирования использвался компонент MasterPLC в Docker контейнере для отображения отредактированной страницы MasterSCADA на странице браузера прокси-сервера nginx.
...
При запуске MasterPLC через ```/etc/init.d/mp1c4 start``` система выдала ошибку: 

```/opt/mp1c4/start_mplc.sh: line 112: /opt/mplc4/nginx/sbin/nginx: No such file or directory```

Это означает, что:

- Nginx отсутствовал по указанному пути ```(/opt/mplc4/nginx/sbin/nginx)```.
- Система пыталась использовать ```yum``` для установки зависимостей, но в образе нет ```yum```.

Как решалась проблема: 

**1. Установка Nginx через пакетный менеджер apt**

```apt-get update && apt-get install -y nginx```

**2. Создание нового каталога для Nginx**
...
После выполненных действий, была произведена проверка работоспособности: 

![image](https://github.com/user-attachments/assets/ef6cdc42-e2d5-4f75-ac6a-df6842c45244)

Как можно заметить на рисунке выше, Команда ```/etc/init.d/mp1c4 start``` завершилась без ошибок:

```Starting MasterPLC...    OK```

```nginx is already running. PID: 148```
...
![image](https://github.com/user-attachments/assets/14a7f9ed-b176-4758-8547-313bb31857fe)



На следующем рисунке представлена конфигурация APM в MasterSCADA для взаимодействия с MasterPLC внутри контейнера:

- Порт 8080 используется для доступа к веб-интерфейсу через прокси-сервер nginx, работающий внутри контейнера.

- Порт 31550 — порт использующийся MasterPLC для обработки соединений по протоколу OPC UA.
...
После загрузки проекта MasterSCADA в MasterPLC веб-интерфейс становится доступен. Однако мы столкнулись с проблемой, которую нужно решить: при остановке и последующем запуске контейнера можно заметить ошибку, что проект не запускается автоматически, веб-интерфейс больше не отображается.
...
При этом службы MasterPLC и nginx продолжают работать.

![image](https://github.com/user-attachments/assets/09929c6b-c61d-413b-a979-4891c2c750bb)

При повторной попытке загрузить проект через MasterSCADA конфигурация не загружается, то есть в самом контейнере проект сохраняется. 
